...
export class Demo extends React.Component {
  static async getInitialProps({ req, query: { transaction } }) {

    // passing :lockid query to the component as a prop
    return { transaction: transaction }
  }

  render() {
    const { transaction, locks } = this.props
    const lock = Object.values(locks).find((lock) => lock.transaction === transaction)
    return(
